...
class OdometerReading extends Component {
    constructor(props) {
        super(props);
        this.state = {
          vehicles: [],
      ...
      sortData = () => {
        setTimeout(() => {
          ...
          this.fetchAllVehiclesByOdoReading(this.state.currentPage);
        }, 500);
      };
      ...
      fetchAllVehiclesByOdoReading(currentPage) {
        currentPage -= 1;
        axios
          .get(
            "http://localhost:8085/fleet-api/vehicles/odometer?pageNumber=" +
              currentPage +
              "&pageSize=" +
            ...
            <Card className={"border border-dark bg-dark text-white"}>
              <Card.Header>
                <div style={{ float: "left" }}>
                  <FontAwesomeIcon icon={faList} /> Average odometer reading grouped by year of manufacture
                </div>
                  ...
                  <tbody>
                    {vehicles.length === 0 ? (
                      ...
                      vehicles.map((vehicle) => (
                        <tr key={vehicle.year}> 
                          <td>{vehicle.year}</td>
                          <td>{vehicle.avgOdometerReading}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
...
export default connect(mapStateToProps) (OdometerReading);